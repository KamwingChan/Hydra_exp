...
class IDMapper:
    ...
    def _load_id_mapping(self, remap_file_path=None):
        ...
        if remap_file_path is None:
            ...
            remap_file_path = env_dir / "config" / "label_remaps" / "behavior_remap.yaml"
        
        if not Path(remap_file_path).exists():
            rospy.logerr(f"Remap file not found: {remap_file_path}")
            rospy.logerr("Please copy behavior_remap.yaml to env/config/label_remaps/behavior_remap.yaml")
