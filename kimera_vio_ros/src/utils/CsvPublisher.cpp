...
void CsvPublisher::callbackOdometry(
  ...
  if (!T_viow_gtw_) {
    ...
    // Simple debug prints to stdout (will appear in terminal)
    std::cout << "\n==== MATRIX ALIGNMENT DEBUG INFO ====\n";
    std::cout << "Points count for alignment: " << src_.cols() << "\n"; 
    std::cout << "Source matrix shape: " << src_.rows() << "x" << src_.cols() << "\n";
    
    // Add basic check for singular matrix condition
    if (src_.cols() < 3) {
      std::cout << "WARNING: Too few points for reliable alignment!\n";
    }
    
    // Check for collinearity which could cause singularity
    if (src_.cols() >= 3) {
      Eigen::Vector3d p1 = src_.col(0);
      Eigen::Vector3d p2 = src_.col(1);
      Eigen::Vector3d p3 = src_.col(2);
      
      Eigen::Vector3d v1 = p2 - p1;
      Eigen::Vector3d v2 = p3 - p1;
      
      double cross_norm = v1.cross(v2).norm();
      std::cout << "Cross product norm (collinearity check): " << cross_norm << "\n";
      if (cross_norm < 1e-3) {
        std::cout << "WARNING: Points might be collinear - could cause 'A is singular' error!\n";
      }
    }
    ...
    try {
      ...
      std::cout << "Umeyama alignment successful!\n";
    }
    catch (const std::exception& e) {
      std::cout << "ERROR in Umeyama alignment: " << e.what() << "\n";
      std::cout << "This is likely the source of 'A is singular' error!\n";
      // Print some matrix details for diagnosis
      std::cout << "First few points of source matrix:\n";
      std::cout << src_.leftCols(std::min(3, (int)src_.cols())) << "\n";
      throw;
    }
